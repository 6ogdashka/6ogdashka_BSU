...
public:
    ...
    void sortReverseAlphabetical() {
        std::sort(M.begin(), M.end(), [](const T& a, const T& b) { if ( a.Section != b.Section) return a.Section < b.Section; else return a.Name > b.Name;});
    }

    QVector<T> search(const QString& name) const {
        QVector<T> foundItems;
