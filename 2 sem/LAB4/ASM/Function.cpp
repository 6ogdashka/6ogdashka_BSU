...
bool IsValidDelimiterString(const char* delimiters) {
    return (delimiters != nullptr && delimiters[0] != '\0');
}
...
char* InputString(const char* msg) {
    ...
    try {
        ...
        if (IsStringEmptyOrWhitespace(buffer)) {
            CleanupMemory(buffer);
            throw InputException("Пустая строка");
        }
        return buffer;
    } catch (...) {
        CleanupMemory(buffer);
...
char* InputDelimiters(const char* msg) {
    ...
    if (!IsValidDelimiterString(delimiters)) {
        ...
        throw InputException("Некорректные разделители");
    }
    return delimiters;
}

char* ExtractWords(const char* input_str, const char* delimiters) {
    if (input_str == nullptr || input_str[0] == '\0' || !IsValidDelimiterString(delimiters)) {
        throw StringException("Ошибка входных данных");
    }
    char* words = AllocateString(kMaxSize + 1);
    try {
        size_t input_index = 0, words_index = 0;
        ...
        if (words_index > 0 && words[words_index - 1] == ' ') words_index--;
        words[words_index] = '\0';
        if (words[0] == '\0') {
            CleanupMemory(words);
            throw StringException("Слова не найдены");
        }
        return words;
    } catch (...) {
        CleanupMemory(words);
...
std::vector<char*> SplitIntoWords(char* words_str) {
    ...
    if (words_str == nullptr || words_str[0] == '\0') return words;
    ...
    try {
        ...
        if (in_word) words.push_back(words_str + word_start);
        return words;
    } catch (...) {
        words.clear();
        throw;
...
char* CopyWord(const char* word) {
    if (!word) throw StringException("Null pointer");
...
int FindMaxUniqueCount(const std::vector<char*>& words) {
    if (words.empty()) throw StringException("Список пуст");
    int max_unique = 0;
...
char* CreateResultString(const std::vector<char*>& words, int max_unique_count) {
    if (words.empty()) throw StringException("Список пуст");
    size_t total_length = 0, word_count = 0;
    for (const char* word : words) {
        if (word && CountUniqueSymbols(word) == max_unique_count) {
            total_length += std::strlen(word);
            word_count++;
        }
    }
    if (word_count == 0) throw StringException("Ничего не найдено");
    ...
    try {
        ...
        for (const char* word : words) {
            if (word && CountUniqueSymbols(word) == max_unique_count) {
                ...
                size_t len = std::strlen(word);
                std::memcpy(result + current_pos, word, len);
                current_pos += len;
                first = false;
            }
        }
